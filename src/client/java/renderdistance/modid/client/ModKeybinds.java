        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (increaseRenderDistance.consumeClick()) {
                ...
                if (current < 32) {
                    ...
                    client.gui.hud.setOverlayMessage(Component.translatable("msg.render-distance.increased", (current + 1)), false);
                } else {
                    client.gui.hud.setOverlayMessage(Component.translatable("msg.render-distance.max"), false);
                }
            }
            while (decreaseRenderDistance.consumeClick()) {
                ...
                if (current > 2) {
                    client.options.renderDistance().set(current - 1);
                    //client.levelRenderer.needsUpdate();
                    client.gui.hud.setOverlayMessage(Component.translatable("msg.render-distance.decreased", (current - 1)), false);
                } else {
                    client.gui.hud.setOverlayMessage(Component.translatable("msg.render-distance.min"), false);
                }
            }
        });
    }
